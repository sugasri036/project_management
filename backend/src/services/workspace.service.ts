...
export const getWorkspaceAnalyticsService = async (workspaceId: string) => {
  ...
  const taskDistribution = await TaskModel.aggregate([
  ...
  const taskPriority = await TaskModel.aggregate([
    { $match: { workspace: new mongoose.Types.ObjectId(workspaceId) } },
    { $group: { _id: "$priority", count: { $sum: 1 } } },
    { $project: { _id: 0, priority: "$_id", count: 1 } }
  ]);

  // Member Performance (Tasks Assigned)
  const memberPerformance = await TaskModel.aggregate([
    { $match: { workspace: new mongoose.Types.ObjectId(workspaceId) } },
  ...
  // Budget & Milestone Analytics
  // 1. Total Planned Budget (Sum of all projects in workspace)
  const budgetStats = await ProjectModel.aggregate([
    { $match: { workspace: new mongoose.Types.ObjectId(workspaceId) } },
    { $group: { _id: null, totalBudget: { $sum: "$budget" } } }
  ]);
  const totalBudget = budgetStats[0]?.totalBudget || 0;

  // 2. Actual Cost (Sum of cost of ALL tasks? Or just DONE tasks? Let's do DONE tasks for "Spent")
  const costStats = await TaskModel.aggregate([
    {
      $match: {
  ...
  const actualSpending = costStats[0]?.totalCost || 0;

  // 3. Milestone Stats
  const totalMilestones = await TaskModel.countDocuments({
    workspace: workspaceId,
    isMilestone: true
  });

  const completedMilestones = await TaskModel.countDocuments({
    workspace: workspaceId,
    isMilestone: true,
    status: TaskStatusEnum.DONE
  });
